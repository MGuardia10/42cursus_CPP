...
void	FragTrap::attack( const std::string& target ) {	
	if (target.empty()) {
		std::cout << this->name << " FragTrap failed to attack, no target specified\n";
		return ;
	}
	if (this->getHP() == 0) {
		std::cout << this->name << " FragTrap cannot attack because it has no HP\n";
		return ;
	}
	if (this->getEP() == 0) {
		std::cout << this->name << " FragTrap cannot attack because it has no EP\n";
		return ;
	}

	this->energyPoints--;

	std::cout << this->name << " FragTrap";
