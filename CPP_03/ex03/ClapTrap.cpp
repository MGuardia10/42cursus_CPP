...
ClapTrap::ClapTrap(): name("Default"), hitPoints(10), energyPoints(10), attackDamage(0), maxHitPoints(10) {
	std::cout << "Default ClapTrap created" << std::endl;
}

ClapTrap::ClapTrap(std::string _name): name(_name), hitPoints(10), energyPoints(10), attackDamage(0), maxHitPoints(10) {
...
void	ClapTrap::attack(const std::string& target) {
	...
	if (target.empty()) {
		std::cout << this->name << " ClapTrap failed to attack, no target specified\n";
		return ;
	}
	if (this->getHP() == 0) {
		std::cout << this->name << " ClapTrap cannot attack because it has no HP\n";
		return ;
	}
	if (this->getEP() == 0) {
		std::cout << this->name << " ClapTrap cannot attack because it has no EP\n";
		return ;
	}

	this->energyPoints--;

	std::cout << this->name << " ClapTrap";
...
void	ClapTrap::takeDamage(unsigned int amount) {
	...
	if (this->getHP() == 0)
		std::cout << this->name << " it's now dead :(" << std::endl;
	else
		std::cout << this->name << " current HP is: " << this->hitPoints << std::endl;
}
...
void	ClapTrap::beRepaired(unsigned int amount) {
	...
	this->energyPoints--;
	(this->hitPoints + amount > this->getMaxHP()) ? this->setHP(this->getMaxHP()) : this->setHP(hitPoints + amount);
	
	std::cout << this->name << " repaired himself successfully!";
...
unsigned int	ClapTrap::getMaxHP( void ) const { return  this->maxHitPoints; }
